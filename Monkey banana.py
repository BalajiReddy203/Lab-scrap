...
def__ str__ (self): 
if not hasattr(self, "locname"): return "unknown" return self.locname 
class HasHeight(Object): 
height:int 
class HasPosition(Object): 
at:Position 
class Monkey(HasHeight,HasPosition): pass class PalmTree(HasHeight,HasPosition): def init (self,*args,**kwargs): 
super().init (*args,**kwargs) 
self.height=2 
class Box(HasHeight,HasPosition): pass class Banana(HasHeight,HasPosition): 
owner:Monkey 
attached:PalmTree
class World(Object):locations:Set[Position]p1=Position() 
p1.locname="Position A" p2=Position() 
p2.locname="Position B" p3=Position() 
...
assert monkey.height<1, "Monkey can only move while on the ground" monkey.at=where 
...
def infer_banana_height(palmtree:PalmTree,banana:Banana):assert banana.attached==palmtree 
...
[go,push,climb_up,grasp,infer_banana_height,infer_owner_at], [w,p1,p2,p3,m,box,p,b], 
